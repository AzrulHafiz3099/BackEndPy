...
def get_students_by_class(class_id: str = Query(...)):
    ...
    try:
        cursor.execute("""
            SELECT Student_ID as student_id, Student_Name as name, Matrix_Number as matrix, Phone_Number as phone 
...
def add_student(student: StudentCreate):
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (next_id, student.class_id, student.name, student.matrix, student.phone))
        conn.commit()
...
def update_student(student_id: str, student: StudentUpdate):
    ...
    try:
        cursor.execute("""
            UPDATE student 
            SET Class_ID=%s, Student_Name=%s, Matrix_Number=%s, Phone_Number=%s
            WHERE Student_ID=%s
        """, (student.class_id, student.name, student.matrix, student.phone, student_id))
        conn.commit()
